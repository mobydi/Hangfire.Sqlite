    ...
    internal class ExpirationManager : IServerComponent
    {
        ...
        public void Execute(CancellationToken cancellationToken)
        {
            foreach (var table in ProcessedTables)
            {
                ...
                do
                {
                    using (var storageConnection = (SQLiteConnection)_storage.GetConnection())
                    {
//                        removedCount = storageConnection.Connection.Execute(
//                            String.Format(@"
//--set transaction isolation level read committed;
//delete from [HangFire.{0}] where ExpireAt < datetime('now', 'utc') limit @count;", table),
                        removedCount = storageConnection.Connection.Execute(
                            String.Format(@"
--set transaction isolation level read committed;
delete from [HangFire.{0}] where ExpireAt < datetime('now', 'utc')", table),
                            new { count = NumberOfRecordsInSinglePass });
                    }
