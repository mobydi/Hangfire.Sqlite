    ...
    public class SQLiteConnectionFacts
    {
        ...
        public void Ctor_ThrowsAnException_WhenConnectionIsNull()
        {
            var exception = Assert.Throws<ArgumentNullException>(
                () => new SQLiteConnection(null, IsolationLevel.Serializable, _providers));
        ...
        {
            var exception = Assert.Throws<ArgumentNullException>(
                () => new SQLiteConnection(ConnectionUtils.CreateConnection(), IsolationLevel.Serializable, null));
        ...
        //    {
        //        var connection = new SQLiteConnection(sqlConnection, IsolationLevel.Serializable, _providers);
        ...
        public void Dispose_DoesNotDisposeTheConnection_IfNotOwned()
        {
            using (var sqlConnection = ConnectionUtils.CreateConnection())
            {
                var connection = new SQLiteConnection(sqlConnection, IsolationLevel.Serializable, _providers, ownsConnection: false);
        ...
        public void CreateExpiredJob_CreatesAJobInTheStorage_AndSetsItsParameters()
        {
            UseConnections((sql, connection) =>
            {
                ...
                Assert.Equal(typeof(SQLiteConnectionFacts), job.Type);
...
values (@invocationData, @arguments, @stateName, datetime('now', 'utc'));
select last_insert_rowid() as Id;";
            ...
            UseConnections((sql, connection) =>
            {
                ...
                Assert.True(DateTime.UtcNow.AddMinutes(-1) < result.CreatedAt.ToLocalTime());
                Assert.True(result.CreatedAt.ToLocalTime() < DateTime.UtcNow.AddMinutes(1));
            });
        }
        ...
        {
            const string arrangeJobSql = @"
insert into [HangFire.Job] (InvocationData, Arguments, StateName, CreatedAt)
values ('', '', '', datetime('now', 'utc'));
select last_insert_rowid() as Id;";

            const string arrangeStateSql = @"
insert into [HangFire.State] (JobId, Name, CreatedAt)
values (@jobId, 'old-state', datetime('now', 'utc'));

insert into [HangFire.State] (JobId, Name, Reason, Data, CreatedAt)
values (@jobId, @name, @reason, @data, datetime('now', 'utc'));

update [HangFire.Job] set StateId = last_insert_rowid()";
...
values (last_insert_rowid(), @name, @value);
select JobId from [HangFire.JobParameter] where Id = last_insert_rowid()";
...
values 
('Server1', '', datetime('now', 'utc')),
('Server2', '', datetime('now', 'utc'))";
...
values (@key, @value, @expireAt, 0.0)";
        ...
        private void UseConnections(Action<IDbConnection, SQLiteConnection> action)
        {
            using (var sqlConnection = ConnectionUtils.CreateConnection())
            using (var connection = new SQLiteConnection(sqlConnection, IsolationLevel.Serializable, _providers))
            {
                action(sqlConnection, connection);
            }
        }

        private void UseConnection(Action<SQLiteConnection> action)
        {
